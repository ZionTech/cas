...
<body role="application" class="bodyLayout">
	<form:form method="post" id="tempForm" commandName="${commandName}"
		htmlEscape="true"></form:form>
	<%
	...
	<input type="hidden" name="defaultTenantLogo"
		value="${defaultTenantLogo}" />
	...
	<header role="banner" id="login-header" class="header">
		<!-- header region -->
	</header>
	<main role="main" class="main">
	<div class="verticalCenterContent" id="login-main"></div>
	</main>
			...
			<form>
				<ul>
					<c:forEach var="entry" items="${pac4jUrls}">
						<li id="${entry.key}"><a href="${entry.value}"
							style="color: #fff">${entry.key}</a></li>
					</c:forEach>
				</ul>
			</form>
		</div>

...
public class TicketsResource {
	...
	public final ResponseEntity<String> createTicketGrantingTicket(
			...
			throws JsonProcessingException {
		/**
		 * TODO: It is not possible to pass around the servlet context through a
		 * hierarcy of changes without making much change in the class
		 * interfaces. Therefore, using threadlocal to maintain the tenant
		 * information. May be it is possible to use spring flow to do that but
		 * this should be a place holder till we figure out the best way to
		 * achieve that.
		 */
		...
		try (Formatter fmt = new Formatter()) {
			...
			for (final Map.Entry<String, Class<? extends Exception>> handlerErrorEntry : e.getHandlerErrors()
					.entrySet()) {
			...
			return new ResponseEntity<>(
					this.jacksonObjectMapper.writer().withDefaultPrettyPrinter().writeValueAsString(errorsMap),
	...
	public final ResponseEntity<String> createServiceTicket(@RequestBody final MultiValueMap<String, String> reqBody,
			@PathVariable("tgtId") final String tgtId) {
		try {

			final String serviceId = reqBody.getFirst(CasProtocolConstants.PARAMETER_SERVICE);
			final AuthenticationContextBuilder builder = new DefaultAuthenticationContextBuilder(
					this.authenticationSystemSupport.getPrincipalElectionStrategy());
			...
			final AuthenticationContext authenticationContext = builder
					.collect(this.ticketRegistrySupport.getAuthenticationFrom(tgtId)).build(service);

			final ServiceTicket serviceTicketId = this.centralAuthenticationService.grantServiceTicket(tgtId, service,
	...
	private static class DefaultCredentialFactory implements CredentialFactory {
		@Override
		public Credential fromRequestBody(@NotNull final MultiValueMap<String, String> reqBody) {
			final MultiValueMap<String, String> requestBody = (MultiValueMap<String, String>) Collections
					.synchronizedMap(reqBody);

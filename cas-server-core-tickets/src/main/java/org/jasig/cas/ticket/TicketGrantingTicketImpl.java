...
public class TicketGrantingTicketImpl extends AbstractTicket implements TicketGrantingTicket {
	...
	private final List<Authentication> supplementalAuthentications = Collections
			.synchronizedList(new ArrayList<Authentication>());
	 ...
	 * @param parentTicketGrantingTicket
	 *            the parent ticket
	 ...
	 */
	public TicketGrantingTicketImpl(final String id, final Service proxiedBy,
			final TicketGrantingTicket parentTicketGrantingTicket, @NotNull final Authentication authentication,
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The state of the ticket is affected by this operation and the ticket will
	 * be considered used. The state update subsequently may impact the ticket
	 * expiration policy in that, depending on the policy configuration, the
	 * ticket may be considered expired.
	 */
	@Override
	public final synchronized ServiceTicket grantServiceTicket(final String id, final Service service,
	 ...
	 *            the only track most recent session
	 */
	protected void updateServiceAndTrackSession(final String id, final Service service,
			final boolean onlyTrackMostRecentSession) {
		updateState();
		...
		if (onlyTrackMostRecentSession) {
			...
			for (final Service existingService : existingServices) {
				...
				// if an existing service has the same normalized path, remove
				// it
	...
	/**
	 * Normalize the path of a service by removing the query string and
	 * everything after a semi-colon.
	 *
	 * @param service
	 ...
	 * ticket-granting ticket. Unlike
	 * {@link java.util.Collections#unmodifiableMap(java.util.Map)}, which is a
	 * view of a separate map which can still change, an instance of
	 * {@link ImmutableMap} contains its own data and will never change.
	 *
